...
% SECOND METHOD - SIDELOBE MINIMIZATION -----------------------------------
% first we specify the sidelobe region \Omega
isSidelobeRegion = zeros(length(angleSpace),1); index = 1;
beamwidth = 5;
for i = -90:90
    if min(abs(i-radAngles(:))) >= beamwidth
        isSidelobeRegion(index) = 1;
    else
        isSidelobeRegion(index) = 0;
    end
    index = index+1;
end
%figure; plot(angleSpaceDeg,isSidelobeRegion); title('Check Target Directions')
...
cvx_begin quiet
    ...
    subject to
        for i = -90:90
            if isSidelobeRegion(index) == 1
                (a1(:,90+radAngles(1))'*R1*a1(:,90+radAngles(1))) - (a1(:,index)'*R1*a1(:,index)) >= t;
                (a1(:,90+radAngles(2))'*R1*a1(:,90+radAngles(2))) - (a1(:,index)'*R1*a1(:,index)) >= t;
            end
            index = index+1;
        end
        (a1(:,90+radAngles(1)-beamwidth)'*R1*a1(:,90+radAngles(1)-beamwidth)) == ...
            (a1(:,90+radAngles(1))'*R1*a1(:,90+radAngles(1)))/2;
        (a1(:,90+radAngles(1)+beamwidth)'*R1*a1(:,90+radAngles(1)+beamwidth)) == ...
            (a1(:,90+radAngles(1))'*R1*a1(:,90+radAngles(1)))/2;
        (a1(:,90+radAngles(2)-beamwidth)'*R1*a1(:,90+radAngles(2)-beamwidth)) == ...
            (a1(:,90+radAngles(2))'*R1*a1(:,90+radAngles(2)))/2;
        (a1(:,90+radAngles(2)+beamwidth)'*R1*a1(:,90+radAngles(2)+beamwidth)) == ...
            (a1(:,90+radAngles(2))'*R1*a1(:,90+radAngles(2)))/2;
